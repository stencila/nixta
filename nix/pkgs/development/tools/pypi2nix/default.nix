{ stdenv, fetchFromGitHub, fetchurl, python37Packages, zip, makeWrapper, nix
, nix-prefetch-git, nix-prefetch-github, nix-prefetch-hg
}:
...
in stdenv.mkDerivation rec {
  ...
  buildInputs = [
    python37Packages.python python37Packages.flake8
    zip makeWrapper nix.out nix-prefetch-git nix-prefetch-github nix-prefetch-hg
  ];
  ...
  patchPhase = ''
    ...
    sed -i -e "s|nix-prefetch-github|${nix-prefetch-github}/bin/nix-prefetch-github|" $out/pkgs/pypi2nix/stage2.py
  ...
  commonPhase = ''
    ...
    echo "#!${python37Packages.python.interpreter}" >  $out/bin/pypi2nix
