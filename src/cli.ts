...
yargs
  ...
  .command(
    ...
    (yargs: any) => {
      yargs
        ...
        .option('docker', {
          describe:
            'Build the environment using Docker? Nix packages are stored on the host.',
          type: 'boolean',
          alias: 'd',
          default: false,
        })
  ...
  .command(
    ...
    (yargs: any) => {
      yargs
        ...
        .option('pure', {
          describe:
            'Should the environment be pure (no host executables available)?',
          alias: 'p',
          type: 'boolean',
        })
    },
  ...
  .command(
    'enter <name> [command..]',
    'Enter a shell within the environment',
    (yargs: any) => {
      yargs
        .positional('name', {
          describe: 'Name of the environment',
  ...
  .command(
    ...
    (yargs: any) => {
      yargs
        ...
        .option('env', {
          describe:
            'The environment to remove packages from (defaults to current)',
          alias: ['e', 'from'],
          type: 'string',
        })
        .env('NIXTA')
    ...
    async (argv: any) => {
      try {
        ...
        info(
          `Removed packages ${pkgs
            .map((pkg: any) => `"${pkg}"`)
            .join(',')} from environment ${env.name}`
        )
      } catch (err) {
        error(err)
      }
    ...
    async (argv: any) => {
      ...
      if (argv.format === 'tsv' || argv.tsv) {
        ...
        for (const row of rows) {
          console.log(
            `${row.name}\t${row.type}\t${row.version}\t${row.description}`
          )
        }
...
function output(
  value: any,
  argv: any = {},
  prettifier?: (value: any) => string
): any {
  if (!(argv.format || argv.json || argv.yaml || argv.pretty)) {
    if (process.stdout.isTTY)
      return output(value, { format: 'pretty' }, prettifier)
